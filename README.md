...
Architecture (ASCII)
  ...
  +----------+            +--------------+            +-----------+
  | HTTP API | <------->  | Worker /     | <------->  | ffmpeg    |
  | (controllers)         | Indexer      |            | (system)  |
  +----------+            +--------------+            +-----------+
		|                        |                         |
		|                        |                         |
		v                        v                         v
  JobRepository (file-backed H2)  Filesystem (mounted directory)
...
sequenceDiagram
	...
	Indexer->>Indexer: ffprobe each .mkv
	Indexer->>Repo: save(PENDING) for matching files
	Worker->>Repo: poll findByStatus("PENDING")
	Worker->>Worker: submit(processJob)
```
...
sequenceDiagram
	...
	Repo->>Worker: PENDING job
	Worker->>FS: move original -> backup
	Worker->>Probe: probe(backup)
	Worker->>FF: ffmpeg convert -> tmpOut
	FF-->>Worker: exit code
	Worker->>Probe: probe(tmpOut)
	Worker->>FS: move tmpOut -> original (atomic)
	Worker->>FS: delete backup
	Worker->>Repo: set DONE/FAILED
```
...
sequenceDiagram
	participant API
	participant Worker
	API->>Worker: POST /api/v1/control/stop
	Worker->>Worker: set running=false (stop polling)
	Note over Worker: let current conversion finish
```
...
sequenceDiagram
	participant API
	participant Repo
	API->>Repo: POST /api/v1/control/clear-index
	Repo-->>API: OK
```
...
```bash
java -Dindex.scanPath=samples -Dffmpeg.threads=1 -Dworker.maxConcurrency=1 -jar build/libs/aac2ac3-service-0.1.0-SNAPSHOT.jar
```

- **Override configuration examples**: `-Dspring.datasource.url=...`, `-Dindex.scanPath=...`, `-Dffmpeg.threads=...`, `-Dworker.maxConcurrency=...`.

- **Note about Maven**: This repository still contains a `pom.xml` for historical reasons; prefer Gradle (`./gradlew`) for day-to-day development. Do not remove `pom.xml` until CI and any consumers are validated.
